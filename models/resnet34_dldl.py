...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes, grayscale, SE):
...
def resnet34(num_classes, grayscale, SE):
...
def cost_fn2(logits, levels):
    val = -torch.sum(levels*(F.logsigmoid(logits) - F.logsigmoid(levels)), dim=1)
    return torch.mean(val)
...
def test(model, data_loader, data, logger, mode):
    ...
    with tqdm(data_loader) as e:
        for inputs in e:
            ...
            pred = torch.sum(logits*rank, dim=1)
...
def train(model_, dataloaders_, optimizer_, lr_scheduler_, num_epochs_, writers, logger, PATH, data):
    ...
    for epoch in range(num_epochs_):
        ...
        for phase in dataloaders_.keys():
            ...
            with tqdm(dataloaders_[phase]) as t:
                for inputs in t:
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        pred = torch.sum(logits*rank, dim=1)
                        loss1 = kl_loss(logits, levels)
                        loss2 = L1_loss(pred, ages)
                        loss = loss1+loss2
                    ...
                    running_kl_loss += loss1.item() * features.size(0)
                    running_l1_loss += loss2.item() * features.size(0)
                    ...
                    # tqdm description
                    t.set_description('{mode} KLLoss={KLLoss:.4f} L1Loss={L1Loss:.4f} loss={loss:.4f} MAE={mae:.4f}'.format(
                                    mode=phase,
                                    L1Loss=running_l1_loss/running_len,
                                    KLLoss=running_kl_loss/running_len,
            ...
            L1_LOSS = running_l1_loss / len(dataloaders_[phase].dataset)
            KL_LOSS = running_kl_loss / len(dataloaders_[phase].dataset)
            ...
            writers.add_scalar(f'{phase}/KL_Loss', KL_LOSS, global_step=epoch)
            writers.add_scalar(f'{phase}/L1_Loss', L1_LOSS, global_step=epoch)

            logger.info('{} Loss: {:.4f} KLLoss: {:.4f} L1Loss: {:.4f} CA_3: {:.4f}, CA_5: {:.4f} MAE:{:.4f}'.format(phase, epoch_loss, KL_LOSS, L1_LOSS, CA_3, CA_5, MAE))
            ...
            if phase == 'valid':
                ...
                logger.info('MAE/CA3/CA5: | Current Valid: %.4f/%.4f/%.4f Ep. %d | Best Valid : %.4f/%.4f/%.4f Ep. %d' % (
...
if __name__ == "__main__":
    model = resnet34(101, False, False)
