...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes, grayscale):
        ...
        self.conv1 = ScatLayer()
        self.conv2 = ScatLayer()
        self.conv3 = ScatLayer()
        self.layer1 = self._make_scat_layer(block, 64, layers[0])
        self.layer2 = self._make_scat_layer(block, 128, layers[1])
        self.layer3 = self._make_scat_layer(block, 256, layers[2])
    ...
    def _make_scat_layer(self, block, planes, blocks, stride=1):
        downsample = None
        if stride != 1 or self.inplanes*7 != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv2d(self.inplanes*7, planes * block.expansion,
                          kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm2d(planes * block.expansion),
            )

        layers = []
        layers.append(block(self.inplanes*7, planes, stride, downsample))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes))
        return nn.Sequential(*layers)
    ...
    def forward(self, x):  # 3*120*120
        x = self.conv1(x)  # 21*60*60
        x1 = self.layer1(x)  # 64*60*60
        # x1 (64, 60, 60)
        x = self.conv2(x1)  # 448*30*30
        x2 = self.layer2(x)  # 128*30*30
        # x2 (128, 30, 30)
        x = self.conv3(x2)  # 896*15*15
        x3 = self.layer3(x)  # 256*15*15
        # x3 (256, 15, 15)
        x = self.layer4(x3)  # 512*8*8
