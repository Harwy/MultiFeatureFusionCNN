...
class MorphDataloader(object):
    def __init__(self, csv_path, img_path, img_size=120, mode='train', data_mode='DLDL', num_classes=101):
...
class MorphDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        age = self.y[index]
        if self.data_mode == 'rank' or self.data_mode == 'ordinal':
            levels = [1]*age + [0]*(self.num_classes - 1 - age)
            levels = torch.tensor(levels, dtype=torch.float32)
        elif self.data_mode == 'dldl':
